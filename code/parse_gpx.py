...
class GPXObj:
    ...
    def __init__(self, gpx_date, gpx_file, phase_file):

        self.geoJson = None
        self.gpx_file = gpx_file
        self.gpx_date = gpx_date
        ...
        # the multiplied duration dur * 1000
        self.duration_multi = []
    ...
    def add_chunk_path(self, path: str):
    ...
    def _parse_gpx(self):
        ...
        all_time = [dt.strptime(timeval, "%Y-%m-%dT%H:%M:%S")
        ...
        for i in range(0, len(all_lat)-1):
            ...
            try:
                ...
                curr_distance = round(
                    mpu.haversine_distance(pt_1, pt_2)*1000, 2)
    ...
    def assignClosestPhasePoint(self):
        ...
        for phloc in phaseLoc:
            ...
            for ll in all_latlon:
                res.append((distance(ll[1], ll[0], float(phloc['lat']), float(
                    phloc['lon'])), ll, (phloc['lat'], phloc['lon'])))
    ...
    def parse_datename_to_json_name(self) -> str:
        ...
        # the month
        filestr += str([month[m]
                       for m in month.keys() if m in file_name_gpx][0])
        # the day
        if len(day) == 1:
            day = '0' + day
        filestr += day
        ...
        # the time
        filestr += str([times[t]
                       for t in times.keys() if t in file_name_gpx][0])
        return filestr
    ...
    def to_geojson(self):
        ...
        # elements
        def elem(duration, phase, file_chunks, chunkscount, lstclusters, distribution, placeloc, gpx_lat_lon): return {
            'type': 'Feature',
            'properties': {
                # from gpx file
                'duration': duration,
                'phase': phase,  # generate from matching gpx filename to phases_dates.csv
                'chunks': file_chunks,
                'chunkscount': chunkscount,
                'lstclusters': lstclusters,
            ...
            'geometry': {
                'type': 'LineString',
                'coordinates': gpx_lat_lon
            }
        }

        def chunks(path, cluster, xy_umap): return {
            'chunkFilepath': path,
            'cluster': cluster,
            'xy_umap': xy_umap
        }
        ...
        res = []
        for i, collection_chunk in enumerate(self.chunks_by_gps):
            file_chunks = []
            lclusters = []
            for chunk in collection_chunk:
                url = chunk['AudioNames']
                cluster = chunk['clusters']
                umap_x = chunk['ft1']
                umap_y = chunk['ft2']
                file_chunks.append(chunks(url, cluster, [umap_x, umap_y]))
                lclusters.append(cluster)

            occurr = Counter(lclusters)
            distribution = {
                k: round(v / total, 2) for total in (sum(occurr.values()),) for k, v in occurr.items()}

            duration = self.duration[i]
            phase = self.phase.split('/')[-1].split('.')[0]

            try:
                gpx_lat_lon = [[self.lon[i], self.lat[i]],
            ...
            res.append(elem(duration, phase, file_chunks,
                       len(file_chunks), list(set(lclusters)), distribution, place, gpx_lat_lon))
        return {'type': 'FeatureCollection', 'features': res[3:-10]}
